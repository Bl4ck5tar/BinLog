...
func ElasticsearchExport() error {
	...
	res, err := global.ESClient.Search().Index(elasticsearch.ArticleIndex()).	//设置查询的索引名称
	...
	for {
		...
		//遍历滚动查询结果中的文档
		for _, hit := range res.Hits.Hits {
			//为每一个文档创建一个 Data 结构体，并将其 ID 和 Source 存储
			data := other.Data{
				ID:		hit.Id_,
				Doc: 	hit.Source_,
			}
			//将数据追加到 response 中的 Data 字段中
			response.Data = append(response.Data, data)
		}
	}
	...
	fileName := fmt.Sprintf("es_%s.json", time.Now().Format("20060101"))
