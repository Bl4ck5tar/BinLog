...
func (userService *UserService) Register(u database.User) (database.User, error) {
	...
	u.Avatar = "server/image/avatar.jpg"
...
func (userService *UserService) Logout(c *gin.Context) {
	...
	global.Redis.Del(uuid.String())		//退出登录时清除用户的缓存
...
//UserFreeze 提供用户暂时冻结操作
func (usrService *UserService) UserFreeze(req request.UserOperation) error {
	var user database.User
