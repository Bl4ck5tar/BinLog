...
func (articleService *ArticleService) ArticleSearch(info request.ArticleSearch) (interface{}, int64, error) {
	...
	//BoolQuery 是复合查询的核心，可以包含：must 必须匹配、should 任意一个匹配、filter 过滤条件
	...
	if info.Category != "" {
		boolQuery.Filter = []types.Query{
			//使用 term 查询，而不是 match，意味着精确匹配，适合枚举类字段
	...
	return global.DB.Transaction(func(tx *gorm.DB) error {
		...
		if errors.Is(tx.Where("user_id = ? AND article_id = ?", req.UserID, req.ArticleID).First(&al).Error, gorm.ErrRecordNotFound) {
			...
			if err := tx.Delete(&al).Error; err != nil {
				global.Log.Error("DB rollback - delete like failed", zap.Error(err))
				return err
			}
			num = -1
		}
...
func (articleService *ArticleService) ArticleCreate(req request.ArticleCreate) error {
	b, err := articleService.Exists(req.Title)
	if err != nil {
		return err
	}
	if b {
...
func (articleService *ArticleService) ArticleUpdate(req request.ArticleUpdate) error {
	...
	return global.DB.Transaction(func(tx *gorm.DB) error {
		...
		if err := utils.InitImagesCategory(tx, removedIllustrations); err != nil {	//释放旧图片
			return err
		}
		if err := utils.ChangeImagesCategory(tx, addedIllustrations, appTypes.Illustration); err != nil {	//新图归类
			return err
		}
