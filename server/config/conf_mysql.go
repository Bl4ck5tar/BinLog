...
type Mysql struct {
	...
	Config		string	`json:"config" yaml:"config"`					//数据库连接的配置参数
	DBName 		string 	`json:"db_name" yaml:"db_name"`					//数据库名称
	Username 	string 	`json:"username" yaml:"username"`				//数据库用户名
	Password	string	`json:"pasword" yaml:"password"`				//数据库密码
	...
	LogMode		string	`json:"log_mode" yaml:"log_mode"`				//日志模式，控制日志输出
}
