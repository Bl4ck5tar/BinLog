...
type JWT struct {
	...
	RefreshTokenSecret	[]byte	//Refresh Token的密钥
}

var (
	TokenExpired = errors.New("token is expired")				//Tokrn 已过期
...
func NewJWT() *JWT {
	return &JWT{
		AccessTokenSecret: []byte(global.Config.Jwt.AccessTokenSecret),		//从全局配置加载
...
func (j *JWT) CreateAccessClaims(baseClaims request.BaseClaims) request.JwtCustomClaims {
	...
	claims := request.JwtCustomClaims{
		BaseClaims: baseClaims,			//基本Claims
...
func (j *JWT) parseToken(tokenString string, claims jwt.Claims, secretKey interface{}) (interface{}, error) {
	...
	if token.Valid {			//Token验证通过，返回 Claims
...
func (j *JWT) ParseAccessToken(tokenString string) (*request.JwtCustomClaims, error) {
	...
	return nil, TokenInvalid		//如果解析结构无效，返回 TokenInvalid 错误
}
