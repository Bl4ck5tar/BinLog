...
func GinLogger() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		global.Log.Info(path,
			zap.Int("status", c.Writer.Status()),									//记录响应状态码
			zap.String("method", c.Request.Method),									//记录请求方法
			zap.String("path", path),												//记录请求路径
			zap.String("query", query),												//记录查询参数
			zap.String("ip", c.ClientIP()),											//记录客户端IP
			zap.String("user-agent", c.Request.UserAgent()),						//记录User-Agent信息
			zap.String("errors", c.Errors.ByType(gin.ErrorTypePrivate).String()),	//记录错误信息
			zap.Duration("cost", cost),												//记录请求耗时
		)
	}
}
	...
	return func(c *gin.Context) {
		...
		defer func() {
			...
			if err := recover(); err != nil {
				//检查是否是连接被断开的问题（如broken pipe），这些错误不需要被记录
				var brokenPipe bool
