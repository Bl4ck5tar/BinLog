...
func (advertisementApi *AdvertisementApi) AdvertisementInfo(c *gin.Context) {
	...
	if err != nil {
		global.Log.Error("Fialed to get advertisement information:", zap.Error(err))
...
func (advertisementApi *AdvertisementApi) AdvertisementList(c *gin.Context) {
	...
	if err != nil {
		global.Log.Error("Failed to get advetisement list:", zap.Error(err))
		response.FailWithMessage("Faield to get advertisement", c)
		return
	}
